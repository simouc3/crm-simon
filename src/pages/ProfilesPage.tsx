        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredProfiles.map((profile) => (
            <div 
              ...
              className="group relative bg-white dark:bg-slate-900 rounded-[40px] border border-border/40 p-8 shadow-sm hover:shadow-xl hover:-translate-y-2 active:scale-95 transition-all duration-500 overflow-hidden cursor-pointer"
            >
              <div className="absolute top-0 right-0 w-32 h-32 bg-primary/5 rounded-full -mr-16 -mt-16 blur-3xl group-hover:bg-primary/10 transition-colors" />
              
              <div className="flex items-start justify-between mb-8">
                <div className="relative">
                  <div className="h-24 w-24 rounded-[30px] bg-slate-50 dark:bg-slate-800 border border-border/20 flex items-center justify-center text-primary overflow-hidden shadow-inner">
                    {profile.avatar_url ? (
                ...
                <div className="text-right">
                  <Badge className={`text-[9px] font-black tracking-widest uppercase h-7 px-4 rounded-xl border-none ${
                    profile.role === 'ADMIN' ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'bg-blue-50 dark:bg-blue-900/20 text-blue-600'
                  }`}>
          ...
          {selectedProfile && (
            <div className="animate-in slide-in-from-bottom-8 duration-500">
              <div className="h-40 bg-primary relative overflow-hidden">
                ...
                <div className="absolute top-6 right-8 text-[10px] font-black text-white/50 uppercase tracking-[0.2em]">Configuración de Usuario</div>
              </div>
              ...
              <div className="p-10 -mt-16 bg-white dark:bg-slate-900 rounded-t-[4rem] relative z-10">
                <div className="flex items-center justify-center -mt-24 mb-6">
                   <div className="relative group">
                      <div className="h-32 w-32 rounded-[45px] bg-white dark:bg-slate-800 p-1 shadow-2xl overflow-hidden">
                         <div className="w-full h-full rounded-[40px] bg-slate-50 dark:bg-slate-700 flex items-center justify-center text-primary overflow-hidden shadow-inner">
                      ...
                      <label className="absolute bottom-0 right-0 h-10 w-10 rounded-2xl bg-primary text-white flex items-center justify-center shadow-xl cursor-pointer hover:scale-110 transition-transform border-4 border-white dark:border-slate-900">
                         <Camera size={18} />
                ...
                <div className="text-center mb-10">
                  ...
                  <div className="flex items-center justify-center gap-2">
                     <Badge className="bg-primary/10 text-primary border-none text-[9px] font-black uppercase tracking-widest px-3">{selectedProfile.role || 'VENTAS'}</Badge>
                ...
                <div className="space-y-8">
                  ...
                  <div className="space-y-4 px-2">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      >
                         <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${selectedProfile.role === 'ADMIN' ? 'bg-primary text-white' : 'bg-slate-100 dark:bg-slate-800'}`}>
                         ...
                         {selectedProfile.role === 'ADMIN' && <div className="w-1.5 h-1.5 rounded-full bg-primary animate-pulse" />}
                      </button>
                      ...
                      <button 
                        ...
                        className={`flex flex-col items-center justify-center gap-3 p-6 rounded-[32px] border-2 transition-all duration-500 ${
                          selectedProfile.role !== 'ADMIN'
                            ? 'border-blue-500 bg-blue-50 dark:bg-blue-950/20 text-blue-500 shadow-xl shadow-blue-500/5' 
                            : 'border-slate-100 dark:border-white/5 text-muted-foreground grayscale opacity-60 hover:grayscale-0 hover:opacity-100'
                        }`}
                      >
                         <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${selectedProfile.role !== 'ADMIN' ? 'bg-blue-500 text-white' : 'bg-slate-100 dark:bg-slate-800'}`}>
                           <User size={22} />
                         </div>
                         <span className="text-[11px] font-black uppercase tracking-widest text-center">Ventas Terreno</span>
                         {selectedProfile.role !== 'ADMIN' && <div className="w-1.5 h-1.5 rounded-full bg-blue-500 animate-pulse" />}
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-4 pt-6">
                    ...
                    <Button onClick={() => setIsDetailOpen(false)} className="flex-1 h-16 rounded-[2rem] font-black uppercase text-xs tracking-widest shadow-2xl shadow-primary/30 active:scale-95 transition-all">
                      Sincronizar Cambios
                    </Button>
                  </div>
                </div>
